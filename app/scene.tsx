...
import {translateAnatomyName,capitalizeName} from './anatomy-names-uz';
interface Props {atlas:Atlas;state:SceneState;lang:Lang;onSelect:(id:string)=>void;onProgress:(n:number)=>void;onError:(s:string)=>void}
export default function AnatomyScene({atlas,state,lang,onSelect,onProgress,onError}:Props){
 const host=useRef<HTMLDivElement>(null),latest=useRef(state),select=useRef(onSelect),langRef=useRef(lang),canvasRef=useRef<HTMLCanvasElement|null>(null);
 latest.current=state;select.current=onSelect;
 useEffect(()=>{langRef.current=lang;if(canvasRef.current)canvasRef.current.setAttribute('aria-label',STRINGS[lang].sceneAriaLabel);},[lang]);
 useEffect(()=>{
  const el=host.current!;let disposed=false,frame=0,dirty=true,ready=false,lastView='',lastReset=-1,lastIsolate='',layoutKey='',amount=0;
  let lastState:SceneState|null=null;
  const abort=new AbortController();
  let renderer:T.WebGLRenderer;
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError(STRINGS[langRef.current].browserNoWebGL);return;}
  ...
  renderer.domElement.setAttribute('aria-label',STRINGS[langRef.current].sceneAriaLabel);
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error(STRINGS[langRef.current].geometryError);geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:STRINGS[langRef.current].loadError);}})();
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError(STRINGS[langRef.current].contextLost);};renderer.domElement.addEventListener('webglcontextlost',contextLost);
  return()=>{disposed=true;abort.abort();cancelAnimationFrame(frame);observer.disconnect();controls.dispose();geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());scene.traverse(o=>{if(o instanceof T.Mesh&&!geometries.includes(o.geometry)){o.geometry.dispose();const ms=Array.isArray(o.material)?o.material:[o.material];ms.forEach(m=>m.dispose());}});env.dispose();partTexture.dispose();selectionTexture.dispose();markerGeometry.dispose();markerMaterial.dispose();hover.remove();renderer.dispose();renderer.domElement.remove();canvasRef.current=null;};
 },[atlas]);
